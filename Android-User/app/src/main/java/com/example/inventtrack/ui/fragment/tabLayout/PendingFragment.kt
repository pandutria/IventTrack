...
import com.example.inventtrack.databinding.FragmentPendingBinding
...
class PendingFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        binding.etSearch.addTextChangedListener(object: TextWatcher{
            ...
            override fun onTextChanged(p0: CharSequence?, p1: Int, p2: Int, p3: Int) {
                showData(this@PendingFragment).execute()
            }
    ...
    class showData(private var fragment: PendingFragment): AsyncTask<Void, Void, Void>() {
        var bookingList: MutableList<Booking> = arrayListOf()
        override fun doInBackground(vararg p0: Void?): Void? {
            try {
                var connection: Connection = dbConnection().getConnection()
                if (connection != null) {
                    var query = "SELECT * FROM [booking] WHERE status = 'pending' AND user_id = ${support.user_id}"

                    if (fragment.binding.etSearch.text.toString() != "") {
                            query = "SELECT * FROM [booking] JOIN [product] ON booking.product_id = product.id WHERE booking.status = 'pending' AND booking.user_id = ${support.user_id} AND product.name LIKE '%${fragment.binding.etSearch.text.toString()}%'"
                    }
